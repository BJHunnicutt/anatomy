function varargout = jh_finalProjMaskAdjustments_green_forKatrina(varargin)
...
% Last Modified by GUIDE v2.5 07-Feb-2015 17:06:10
...
% % % load str/green_str_MaskedOutProjections.mat  %JH - Removed For Katrina's step
b = pwd;
brain = b(end-5:end);
...
if ~isfield(projections.left, 'green')
    projections.left.green.start = input('What is the FIRST slice with GREEN projections on the LEFT?');
    projections.left.green.end = input('What is the LAST slice with GREEN projections on the LEFT?');    
    projections.right.green.start = input('What is the FIRST slice with GREEN projections on the RIGHT?');
    projections.right.green.end = input('What is the LAST slice with GREEN projections on the RIGHT?');
    save('str/strdata','projections','-append');
end

if exist(['str/wekaProbMaskGreen','_Threshold', num2str(WEKA.threshold.greenHigh,'%02i'), '.mat']);   % loading the probability masks
    load (['str/wekaProbMaskGreen','_Threshold', num2str(WEKA.threshold.greenHigh,'%02i'), '.mat'], 'wekaProbMaskGreen');
    wekaProbMaskHigh = wekaProbMaskGreen;
    load (['str/wekaProbMaskGreen','_Threshold', num2str(WEKA.threshold.greenLow,'%02i'),'.mat'], 'wekaProbMaskGreen');
    wekaProbMaskLow = wekaProbMaskGreen;
    handles.probStatus = 1;   % storing a handle to say whether or not green projections exist for this brain
else
...
if ~exist('str/green_addmask.mat','file')         %initializing the final additions/subtractions that i will make to the masks
    addmask(2500,3500,strnd-strstrt+1)= false; 
    save('str/green_addmask.mat','addmask')
else
    load str/green_addmask.mat
end

if ~exist('str/green_str_MaskedOutProjections.mat','file')         %initializing the final additions/subtractions that i will make to the masks
    colormask(2500,3500,strnd-strstrt+1)= false; 
    save('str/green_str_MaskedOutProjections.mat','colormask')
else 
...
set(handles.checkbox1, 'string', ['High Green Threshold: ', num2str(WEKA.threshold.greenHigh)]) % Displaying the low threshold
set(handles.checkbox2, 'string', ['LOW Green Threshold: ', num2str(WEKA.threshold.greenLow)]) % Displaying the low threshold
set(handles.edit2, 'string', num2str(projections.left.green.start)); % Displaying the left green projection start
set(handles.edit3, 'string', num2str(projections.left.green.end)); % Displaying the left green projection end
set(handles.edit4, 'string', num2str(projections.right.green.start)); % Displaying the right green projection start
set(handles.edit5, 'string', num2str(projections.right.green.end)); % Displaying the right green projection end
...
nano(:,:,1)= nano(:,:,1) + uint16(wekaProbMaskHigh(:, :, k-strstrt+1))*2^16;   % Plot the low threshold cyan and high white
nano(:,:,2)= nano(:,:,2) + uint16(wekaProbMaskLow(:, :, k-strstrt+1))*2^16;
...
for j = 1:length(addOutline)
    plot((addOutline{j}(:,2)), (addOutline{j}(:,1))+0.5, 'g--', 'linewidth',2)
end
hold off
...
handles.HighThresh= WEKA.threshold.greenHigh;
handles.LowThresh = WEKA.threshold.greenLow;
...
if ~isfield(WEKA.thresholdBySlice, 'green')
    WEKA.thresholdBySlice.green = zeros(1, length(masteralign));
    WEKA.thresholdBySlice.green(:) = WEKA.threshold.greenLow;
end
if WEKA.thresholdBySlice.green(k) == handles.LowThresh;
    set(handles.checkbox2,'Value',1); % Low Threshold Checkbox ON
    set(handles.checkbox1,'Value',0); % High Threshold Checkbox OFF
elseif WEKA.thresholdBySlice.green(k) == handles.HighThresh;
...
handles.thresholdBySlice = WEKA.thresholdBySlice.green;
...
if k > strnd
    save('str/green_addmask.mat', 'addmask')
    save('str/green_str_MaskedOutProjections.mat', 'colormask') % Save up here, so if there is an error in the slice number it saves first!
    c = clock;
...
WEKA.thresholdBySlice.green = handles.thresholdBySlice;
if WEKA.thresholdBySlice.green(k) == handles.HighThresh 
...
elseif WEKA.thresholdBySlice.green(k) == handles.LowThresh 
...
if get(hObject,'Value') == 1  % If "View Result" checkbox is turned ON
    ...
    if WEKA.thresholdBySlice.green(k) == WEKA.threshold.greenHigh;
    ...
    elseif WEKA.thresholdBySlice.green(k) == WEKA.threshold.greenLow;
...
% --- Change the START section for LEFT Green Projections
...
projections.left.green.start = k;
...
% --- Change the END section for LEFT Green Projections
...
% --- Change the START section for RIGHT Green Projections
...
projections.right.green.start = k;
...
% --- Change the END section for RIGHT Green Projections
